...
/* WMMA F16 prefill matmul.  Mirrors rocm_matmul_q8_0_wmma_kernel but the
 * A operand is dense F16 weights — no Q8 unpacking, no per-block scale.
 * One 32-lane wave produces a 16-row x 16-token output tile via
 * v_wmma_f32_16x16x16_f16 (one WMMA per 16 K-elements).  Constraints:
 * in_dim multiple of 16; out_dim and n_tok need not be (kernel masks at
...
static void rocm_matmul_f16_wmma_kernel(
    ...
    /* Clamp source pointers so out-of-range lanes (partial output tiles)
     * still read valid memory.  Their results are filtered at store. */
    const uint32_t safe_row = (out_row < out_dim) ? out_row : (out_dim - 1u);
    const uint32_t safe_tok = (tok     < n_tok)   ? tok     : (n_tok   - 1u);
...
int ds4_metal_matmul_f16_tensor(
    ...
    if (n_tok > 1u) {
        ...
        if (g_f16_wmma_cached < 0) {
            ...
            g_f16_wmma_cached = (env && env[0] == '0') ? 0 : 1;
        }
...
__global__ static void rocm_attn_fa_kernel_##SUFFIX( \
        ...
        int mode_static) { \
    extern __shared__ float smem[]; \
    const uint32_t BR     = (uint32_t)(BR_VAL); \
    float *q_lds          = smem; \
...
static int rocm_attn_batch_dispatch(
     ...
     * wave32 multiple.
     *
     * BR=16 amortizes the per-K cooperative LDS load and the memory-latency
     * stall across 4x more rows than BR=4.  Measured +6% prefill end-to-end
     * (51 -> 54 t/s on the 700-word benchmark) by cutting attention_prefill
     * from 58 -> 36 ms/call.  LDS budget at BR=16: (16+1)*head_dim*4 bytes
     * = 34 KiB at head_dim=512, well under the 64 KiB workgroup limit.
     * BR=16 needs n_tokens >= 16 for non-trivial occupancy; smaller batches
     * (decode-batch with speculative depth, masked prefill at the chunk
     * tail) stay on BR=4. */
    const int fa_ok = !use_topk &&
                      head_dim >= 32u && (head_dim % 32u) == 0u &&
                      (head_dim / 32u) <= 32u;
    if (fa_ok) {
        const int use_br16 = (n_tokens >= 16u);
        ...
        if (use_br16 && dpl == 16u) {
            ...
            hipLaunchKernelGGL(rocm_attn_fa_kernel_br4, grid, block, (uint32_t)smem_bytes, g_stream,
                               (float *)tensor_u8(heads),
                               (const float *)tensor_u8_const(q),
                               (const float *)tensor_u8_const(raw_kv),
                               n_comp ? (const float *)tensor_u8_const(comp_kv) : (const float *)NULL,
...
/* WMMA-gather routed-MoE prefill kernels (IQ2_XXS gate/up, Q2_K down).
 *
 * Each block produces a 16-row x 16-pair output tile via one wave (32 lanes)
 * issuing v_wmma_f32_16x16x16_f16 instructions, after dequantizing weight
 * tiles in registers on the fly.  Pair tile semantics follow the EM scheduler:
 *   - blockIdx.y       = unique expert index (dense)
 *   - blockIdx.x       = mid-dim row tile (16-wide)
 *   - inner pair loop  = ceil(pair_count_for_this_expert / 16)
 *
 * Grid order (mid_row_tile, expert) means HIP dispatches blocks for the same
 * expert close in time — the expert's pair-tile activations (16 tokens x
 * in_dim floats = 256 KiB at in_dim=4096) fit in L2 and amortize across all
 * 128 row-tile blocks for that expert.
 *
 * vs legacy wave-n8: weights are read pair_count/16 ≈ 2 times per (expert,
 * row) instead of pair_count ≈ 30 times.  Compute is dense WMMA F16 vs the
 * scalar lane-per-row dequant+fma loop.
 *
 * Layout invariants (same as rocm_matmul_q8_0_wmma_kernel, verified there):
 *   A tile (16 rows x 16 K): lane L = (L%16) provides row's K-vector.
 *     Lanes 16..31 broadcast lanes 0..15.
 *   B tile (16 K x 16 cols): lane L = (L%16) provides col's K-vector.
 *     Lanes 16..31 broadcast.
 *   C tile (16 rows x 16 cols): lane L holds 8 results at rows
...
/* Persistent variant with LDS-staged iq2xxs grid + ksigns.  Same body as
 * rocm_moe_gate_up_iq2xxs_wmma_persistent_kernel; LDS staging happens
 * once at block start (outside the grid-stride tile loop) so the cost
 * amortizes across every tile this persistent block handles. */
__global__ __launch_bounds__(32, 8)
static void rocm_moe_gate_up_iq2xxs_wmma_persistent_lds_grid_kernel(
        float *mid,
        const uint8_t *gate_pool,
        const uint8_t *up_pool,
        uint64_t gate_expert_bytes,
...
/* I8 variant of the gate_up kernel with iq2xxs grid + ksigns staged into
 * LDS (mirrors rocm_moe_gate_up_iq2xxs_wmma_lds_grid_kernel for the F16
 * path).  Per feedback_wmma_i8_path.md, the I8 MoE bottleneck is
 * memory-instruction throughput, which is exactly what LDS-staging
 * attacks. */
__global__ __launch_bounds__(32, 8)
static void rocm_moe_gate_up_iq2xxs_wmma_i8_lds_grid_kernel(
        float *mid,
        const uint8_t *gate_pool,
        const uint8_t *up_pool,
        uint64_t gate_expert_bytes,
    ...
    for (uint32_t pair_base = pair_start; pair_base < pair_end; pair_base += 16u) {
        ...
        for (uint32_t sub = 0; sub < total_subs; sub++) {
            ...
            _Pragma("unroll")
            for (int j = 0; j < 8; j++) {
                a_gate1[j]      = (int8_t)((int)(uint8_t)(g0u64 >> (j * 8)) * (((gs0 >> j) & 1u) ? -1 : 1));
                a_gate1[j + 8]  = (int8_t)((int)(uint8_t)(g1u64 >> (j * 8)) * (((gs1 >> j) & 1u) ? -1 : 1));
                a_gate2[j]      = (int8_t)((int)(uint8_t)(g2u64 >> (j * 8)) * (((gs2 >> j) & 1u) ? -1 : 1));
                a_gate2[j + 8]  = (int8_t)((int)(uint8_t)(g3u64 >> (j * 8)) * (((gs3 >> j) & 1u) ? -1 : 1));
            }
            ...
            _Pragma("unroll")
            for (int j = 0; j < 8; j++) {
                a_up1[j]      = (int8_t)((int)(uint8_t)(u0u64 >> (j * 8)) * (((us0 >> j) & 1u) ? -1 : 1));
                a_up1[j + 8]  = (int8_t)((int)(uint8_t)(u1u64 >> (j * 8)) * (((us1 >> j) & 1u) ? -1 : 1));
                a_up2[j]      = (int8_t)((int)(uint8_t)(u2u64 >> (j * 8)) * (((us2 >> j) & 1u) ? -1 : 1));
                a_up2[j + 8]  = (int8_t)((int)(uint8_t)(u3u64 >> (j * 8)) * (((us3 >> j) & 1u) ? -1 : 1));
            }
 ...
 * weights as F16 with the q2_K offset baked in (w = d*scale*q - dmin*minv),
 * then runs one WMMA against the lane-column's mid[] slice.
 *
...
static void rocm_moe_down_q2k_wmma_kernel(
    ...
    for (uint32_t pair_base = pair_start; pair_base < pair_end; pair_base += 16u) {
        ...
        for (uint32_t qb = 0; qb < total_qb; qb++) {
            ...
            const float    scale    = (float)(sc & 0x0F);
            const float    minv     = (float)(sc >> 4);
            const float    offset_f = -dmin * minv;       /* added to each weight */
...
/* tsk-83: variant of rocm_moe_down_q2k_wmma_lds_scales_kernel that ALSO
 * cooperatively stages the 64-byte qs payload per Q2_K block into LDS.
 * Lanes L and L+16 share down_row_p (safe_row = row_base + (lane & 15)), so
 * qs reads are 2x redundant per K-block.  Stage qs once: lanes 0..15 stage
 * bytes 0..31 of their row; lanes 16..31 stage bytes 32..63 of row=lane-16.
 * Row stride padded to 17 dwords (68 B) to avoid 8-way LDS bank conflicts. */
__global__ __launch_bounds__(32, 8)
static void rocm_moe_down_q2k_wmma_lds_scales_qs_kernel(
        float *out,
        const uint8_t *down_pool,
        uint64_t down_expert_bytes,
        uint64_t down_row_bytes,
    ...
    __shared__ float    lds_dmin[16];
    __shared__ uint32_t lds_scales[16][4];
    __shared__ uint32_t lds_qs[16][17];   /* 16 rows x 16 dwords, padded to 17 */
    ...
    for (uint32_t pair_base = pair_start; pair_base < pair_end; pair_base += 16u) {
        ...
        for (uint32_t bi = 0; bi < nb; bi++) {
            /* Cooperative staging: lanes 0..15 stage d/dmin/scales + qs
             * dwords 0..7 (bytes 0..31).  Lanes 16..31 stage qs dwords 8..15
             * (bytes 32..63) for row=lane-16 — same down_row_p (rows are
             * lane-mod-16). */
            const rocm_block_q2_K *xb_row = down_row_p + bi;
            if (lane < 16u) {
                lds_d[lane]    = rocm_f16_to_f32(xb_row->d);
                lds_dmin[lane] = rocm_f16_to_f32(xb_row->dmin);
                const uint32_t *src_sc = (const uint32_t *)xb_row->scales;
                lds_scales[lane][0] = src_sc[0];
                lds_scales[lane][1] = src_sc[1];
                lds_scales[lane][2] = src_sc[2];
                lds_scales[lane][3] = src_sc[3];
                const uint32_t *src_qs = (const uint32_t *)xb_row->qs;
                lds_qs[lane][0] = src_qs[0];
                lds_qs[lane][1] = src_qs[1];
                lds_qs[lane][2] = src_qs[2];
                lds_qs[lane][3] = src_qs[3];
                lds_qs[lane][4] = src_qs[4];
                lds_qs[lane][5] = src_qs[5];
                lds_qs[lane][6] = src_qs[6];
                lds_qs[lane][7] = src_qs[7];
            } else {
                const uint32_t row_l = lane - 16u;
                const uint32_t *src_qs = (const uint32_t *)xb_row->qs;
                lds_qs[row_l][8]  = src_qs[8];
                lds_qs[row_l][9]  = src_qs[9];
                lds_qs[row_l][10] = src_qs[10];
                lds_qs[row_l][11] = src_qs[11];
                lds_qs[row_l][12] = src_qs[12];
                lds_qs[row_l][13] = src_qs[13];
                lds_qs[row_l][14] = src_qs[14];
                lds_qs[row_l][15] = src_qs[15];
            }
            __syncthreads();
            ...
            const float    dmin = lds_dmin[row_in_tile];
            const uint8_t *lds_sc_row = (const uint8_t *)&lds_scales[row_in_tile][0];
            const uint8_t *lds_qs_row = (const uint8_t *)&lds_qs[row_in_tile][0];
            ...
            for (uint32_t inn = 0; inn < 16u; inn++) {
                ...
                const float    offset_f = -dmin * minv;
                const float    coef_f   = d * scale;
                const uint8_t *q        = lds_qs_row + k * 32u + half * 16u;
...
/* tsk-83: persistent + LDS-staged (scales + qs) variant.  Grid-stride
 * scheduling of rocm_moe_down_q2k_wmma_lds_scales_qs_kernel. */
__global__ __launch_bounds__(32, 8)
static void rocm_moe_down_q2k_wmma_persistent_lds_scales_qs_kernel(
        float *out,
        const uint8_t *down_pool,
        uint64_t down_expert_bytes,
        uint64_t down_row_bytes,
    ...
    __shared__ float    lds_dmin[16];
    __shared__ uint32_t lds_scales[16][4];
    __shared__ uint32_t lds_qs[16][17];
...
static int rocm_moe_run(
    ...
    /* WMMA-gather fast path for prefill (n_tokens >= threshold) on the
     * IQ2_XXS gate/up + Q2_K down quant combo.  Uses the same scheduler as
     * the EM path but runs the gate_up via 16x16x16 WMMA F16 (each lane
     * dequants its row's IQ2_XXS blocks and feeds them into v_wmma_*).  The
     * down kernel stays at legacy wave-n8 in this checkpoint; that doubles
     * up reading mid[] but lets us validate gate_up first.
     *
     * Grid order (mid_row_tile, n_unique) groups same-expert blocks
     * temporally; the expert's pair-tile activation slab (16 tokens x
     * in_dim floats = 256 KiB at in_dim=4096) fits in L2 and amortizes
     * across the n_unique mid_row tiles.
     *
     * Enabled by default; set DS4_ROCM_MOE_WMMA=0 to disable. */
    static int g_wmma_moe_cached = -1;
    if (g_wmma_moe_cached < 0) {
        const char *env = getenv("DS4_ROCM_MOE_WMMA");
        g_wmma_moe_cached = (env && env[0] == '0') ? 0 : 1;
    }
    /* WMMA tiles 16 pairs at a time.  Pair density = n_tokens * n_used / 256
     * unique experts; below ~4 pairs/expert the 16-wide tile is too sparse to
     * recover the scheduler + per-tile setup cost (measured 10.7 vs 7.5
     * ms/call at n_tokens=24 — 43% regression).  At n_tokens>=128 the density
     * is >=4 pairs/expert and WMMA wins decisively (101 vs 159 ms/call at
     * n_tokens=855, +27% prefill). */
    const int wmma_moe_path = g_wmma_moe_cached &&
                              (n_tokens >= 128u) &&
                              (gate_type == DS4_TENSOR_TYPE_IQ2_XXS) &&
                              (up_type   == DS4_TENSOR_TYPE_IQ2_XXS) &&
                              (down_type == DS4_TENSOR_TYPE_Q2_K) &&
                              ((expert_mid_dim & 15u) == 0u) &&
                              ((expert_in_dim % DS4_QK_K) == 0u) &&
                              ((expert_mid_dim % DS4_QK_K) == 0u) &&
                              ((out_dim & 15u) == 0u);
    if (wmma_moe_path) {
        const uint32_t max_dense   = n_tokens * n_used;
        const size_t   hdr_bytes   = sizeof(uint32_t) * (2u + (uint32_t)total_experts + (max_dense + 1u));
        ...
        if (rocm_moe_em_scratch_ensure(need_bytes)) {
            const uint32_t sched_block      = 256u;
            const size_t   sched_smem       = sizeof(uint32_t) * (2u * (uint32_t)total_experts + max_dense);
            const uint32_t gu_row_blocks    = (expert_mid_dim + 15u) / 16u;
            ...
            hipLaunchKernelGGL(rocm_moe_group_pairs_kernel,
                               ...
                               (const float   *)tensor_u8_const(weights),
                               n_tokens, n_used, (uint32_t)total_experts, max_dense);
            int wm_ok = rocm_launch_done("moe wmma schedule launch", "moe wmma schedule completion");

            /* Cap grid at total_experts: n_unique <= 256 always (= pool size),
             * and the scheduler pads expert_pair_offsets out to max_dense so
            ...
            /* Shared LDS-grid cache: governs whether all WMMA gate_up variants
             * (F16, I8, persistent) stage the iq2xxs grid + ksigns into LDS.
             * Default ON; DS4_ROCM_MOE_LDS_GRID=0 reverts. */
            ...
            if (wm_ok && gu_grid_y > 0u && g_moe_wmma_i8_cached
                ...
                if (rocm_act_q8_scratch_ensure(scratch_bytes)) {
                    ...
                    if (g_moe_lds_grid_cached) {
                        hipLaunchKernelGGL(rocm_moe_gate_up_iq2xxs_wmma_i8_lds_grid_kernel,
                                           ...
                                           sched_pair_token, sched_pair_slot, sched_pair_weight,
                                           expert_in_dim, expert_mid_dim, n_used, clamp);
                        wm_ok = rocm_launch_done("moe wmma_i8_lds_grid gate_up launch",
                                                 "moe wmma_i8_lds_grid gate_up completion");
                    } else {
            ...
            if (wm_ok && gu_grid_y > 0u && !dispatched_gu_i8
                ...
                if (g_moe_lds_grid_cached && g_moe_lds_w_cached) {
                    ...
                    hipLaunchKernelGGL(rocm_moe_gate_up_iq2xxs_wmma_persistent_lds_grid_kernel,
                                       ...
                                       expert_in_dim, expert_mid_dim, n_used,
                                       gu_row_blocks, gu_grid_y, clamp);
                    wm_ok = rocm_launch_done("moe wmma_persistent_lds_grid gate_up launch",
                                             "moe wmma_persistent_lds_grid gate_up completion");
                } else {
             ...
             * to revert.
             * DS4_ROCM_MOE_DOWN_LDS_QS: stage the 64-byte qs payload per
             * Q2_K block into LDS (in addition to the d/dmin/scales header).
             * Lanes 0..15 share down_row_p with lanes 16..31 so qs reads are
             * 2x redundant per K-block; co-staging once amortizes the load.
             * Default ON — microbench (n_tok=184) shows down kernel
             * 5.67 -> 4.79 ms/call (-15.6%) and routed-MoE wall 11.0 -> 10.1
             * ms/call (-8.2%); 40/40 kernel tests pass.  Set =0 to revert. */
            static int g_moe_down_persist_cached = -1;
            static int g_moe_down_lds_cached     = -1;
            static int g_moe_down_lds_qs_cached  = -1;
            ...
            if (g_moe_down_lds_qs_cached < 0) {
                const char *env = getenv("DS4_ROCM_MOE_DOWN_LDS_QS");
                g_moe_down_lds_qs_cached = (env && env[0] == '0') ? 0 : 1;
            }
            if (wm_ok && gu_grid_y > 0u) {
                ...
                if (g_moe_down_persist_cached) {
                    ...
                    if (g_moe_down_lds_cached && g_moe_down_lds_qs_cached) {
                        hipLaunchKernelGGL(rocm_moe_down_q2k_wmma_persistent_lds_scales_qs_kernel,
                        ...
                        wm_ok = rocm_launch_done("moe wmma_persistent_lds_scales_qs down launch",
                                                 "moe wmma_persistent_lds_scales_qs down completion");
                ...
                } else if (g_moe_down_lds_cached && g_moe_down_lds_qs_cached) {
                    dim3 dn_grid(dn_row_blocks, gu_grid_y, 1);
                    hipLaunchKernelGGL(rocm_moe_down_q2k_wmma_lds_scales_qs_kernel,
                                       dn_grid, dn_block, 0, g_stream,
                                       (float *)tensor_u8(out),
                                       (const uint8_t *)down_d,
                                       down_expert_bytes, down_row_bytes,
                                       ...
                                       sched_pair_token, sched_pair_slot,
                                       expert_mid_dim, out_dim, n_used);
                    wm_ok = rocm_launch_done("moe wmma_lds_scales_qs down launch",
                                             "moe wmma_lds_scales_qs down completion");
                } else if (g_moe_down_lds_cached) {
                    dim3 dn_grid(dn_row_blocks, gu_grid_y, 1);
